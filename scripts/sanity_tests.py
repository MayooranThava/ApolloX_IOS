...
def main() -> int:
    ...
    check("startBackgroundMusicIfNeeded" in audio, "background music loop missing")
    check("backgroundMusicLoop" in audio, "background music asset key missing")
    check("AppSettings.sfxVolume" in audio, "AudioManager must honor SFX volume")
    ...
    check("firstBoss" in achievements and "score500" in achievements and "fiveLives" in achievements,
          "core achievements missing")
    check("NEW BEST" in over, "game over should celebrate new best")
    check("rankDeltaLabel" in over or "RANK" in over, "game over should show rank delta")
    check("lastCommitWasNewBest" in score, "ScoreStore should track new-best runs")
    check("loadLocalPlayerRank" in gc, "Game Center should fetch local player rank")
    check((ROOT / "docs" / "app-review-notes.md").exists(), "App Review notes missing")
    bg_json = (ROOT / "ApolloX" / "Assets.xcassets" / "background.imageset" / "Contents.json").read_text()
    check("background.png" in bg_json, "background imageset missing")
    check("SKTexture.preload" in (ROOT / "ApolloX" / "TextureCache.swift").read_text(),
          "TextureCache should batch-preload sprites (atlas-style)")
    check((ROOT / "ApolloX" / "backgroundMusicLoop.wav").exists(), "backgroundMusicLoop.wav missing")
